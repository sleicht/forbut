...
#
# Function ordering mirrors forgit/cmds/diff.sh:
#   _forbut_diff_preview         →  preview helper (≈ _forgit_diff_view-as-preview)
#   _forbut_diff_enter           →  enter helper   (≈ _forgit_diff_enter)
#   _forbut_diff_rows_from_json  →  jq row builder (forbut-specific)
#   _forbut_diff_target_view     →  target-mode rows (forbut-specific)
#   _forbut_diff_view            →  unified row source (≈ _forgit_diff_view)
#   _forbut_diff                 →  main entry     (≈ _forgit_diff) — LAST
...
# (analogous to forgit's _forgit_diff_view: assembles the row source
# the main entry feeds into fzf).
# ---------------------------------------------------------------------------
_forbut_diff_view() {
    local target="${1:-}"
    ...
    if [[ -n $target ]]; then
        _forbut_diff_target_view "$target"
        return
    fi
...
# ---------------------------------------------------------------------------
# git diff viewer (main entry — comes LAST, mirroring forgit)
# ---------------------------------------------------------------------------
_forbut_diff() {
    local target="${1:-}"
