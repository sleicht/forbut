...
| `git checkout <branch>` | `forgit::checkout_branch` | `but apply` / `but unapply` | `forbut::switch` | v0.1 |
...
| `git stash` | `forgit::stash` | *(replaced by virtual branches)* | -- | N/A |
| `git cherry-pick` | `forgit::cherry_pick` | `but pick` | -- | -- |
...
Browse stacks and their commits with preview. GitButler's virtual branches
